...
function fakeSdk(overrides: { readyDevice?: string; failWith?: string } = {}) {
  const player = {
    ...
    connect: vi.fn(async () => {
      ...
      const payload = overrides.failWith ? {} : { device_id: overrides.readyDevice ?? 'device-1' };
  ...
  const sdk = { Player: vi.fn(function Player() {
    return player;
  }) } as unknown as never;
  return { sdk, player };
}

function make(overrides: Parameters<typeof fakeSdk>[0] = {}, fetchImpl?: typeof fetch) {
  const { sdk, player } = fakeSdk(overrides);
  const playback = new SpotifyWebPlayback({
    getToken: async () => 'token-abc',
    loadSdk: async () => sdk,
    fetchImpl: fetchImpl ?? (vi.fn(async () => new Response(null, { status: 204 })) as typeof fetch),
  });
...
describe('SpotifyWebPlayback', () => {
  ...
  it('reports a Premium requirement instead of a generic failure', async () => {
    ...
    await expect(playback.start({ uri: 'spotify:track:a', position_ms: 0 })).rejects.toMatchObject({
      kind: 'premium',
    });
  });
  ...
  it('reports an expired session as an auth problem', async () => {
    ...
    await expect(playback.start({ uri: 'spotify:track:a', position_ms: 0 })).rejects.toBeInstanceOf(
      PlaybackError,
    );
  });
